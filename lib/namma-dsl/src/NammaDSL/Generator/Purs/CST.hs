...
addNewDecl recType recName enumDef =
   ...
   in case recType of
        ...
        PDATA -> case enumDef of
          Just enum
            ...
            | otherwise -> DeclData () dh (Just (eqTok, parseEnumConstructors (addSpacesToEnum enum)))
  ...
  where
    ...
    addSpacesToEnum = T.intercalate " | " . map T.strip . T.splitOn ","

    parseDataCtor ctor =
      let (name, types) = T.break isSpace (T.strip ctor)
          ctorName = pName [] [Space 1] (pToken name) (ProperName name)
          ctorTypes =
