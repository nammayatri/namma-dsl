...
appServerUsesPublicApi apiRead apiT
  | not (appServerDashboardAuth apiRead) = False
  | apiT ^. apiAppServerCustomHandler = True
  | Nothing <- apiT ^. apiHelperApi = False
  | null unsafeUnits = True
  | otherwise =
    error $
      "Cannot generate a safe application server handler for "
        <> T.unpack (handlerFunctionText apiT)
        <> ": its helperApi differs from the public API in "
        <> intercalate ", " unsafeUnits
        <> ", which the generated handler cannot fill from the verified operator's session."
        <> " Serving the helperApi would let the client set these values."
        <> " Add `appServerHandler: custom` and a hand-written handler under "
        <> appServerCustomHandlerModulePrefix apiRead
        <> "."
  where
    publicUnits = mkApiSignatureUnits apiT
    helperUnits = mkApiSignatureUnitsHelper apiT
    describe u = unitName u <> " :: " <> T.unpack (apiSignatureType u)
    unsafeUnits =
      [ describe u
        | u <- helperUnits,
          case find ((== unitName u) . unitName) publicUnits of
            Just pu -> apiSignatureType pu /= apiSignatureType u
            Nothing -> not (apiHasOperatorArg apiT && isJust (sessionDerivedParam u))
      ]
        <> [ describe pu <> " (public only)"
             | pu <- publicUnits,
               unitName pu `notElem` (unitName <$> helperUnits)
           ]
  ...
  where
    ...
    handlerFunctionDef moduleName' apiT = do
      let functionName = handlerFunctionText apiT
          ...
          -- Only Helper parameters missing from the public shape come from the
          -- session; with none, both shapes take the same arguments.
          useSessionArgs = usePublic && not (null (helperExtraUnits apiT))
